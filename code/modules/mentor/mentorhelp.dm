/client/verb/mentorhelp(msg as text)
	...
	var/mentor_msg = "<span class='mentornotice'><b><span class='mentorhelp'>MENTORHELP:</b> <b>[key_name_mentor(src, 1, 0, 1, show_char)]</b> (<a href='?_src_=mentor;[MentorHrefToken(TRUE)];mentor_friend=[REF(src.mob)]'>IF</a>) : [msg]</span></span>"
	...
	if(GLOB.mentors.len)
		send2chat("Mentorhelp from [key_name_mentor(src)]: [msg]\nThere are [GLOB.mentors.len] online able to respond.", "mhelp")
	else
		send2chat("Mentorhelp from [key_name_mentor(src)]: [msg]\nThere are no mentors online.", "mhelp")
	return
